...
export default {
  data() {
    return {
      operation: 0, // 0为打卡，1为复习
      currentQuestionIndex: 0,
      selectedOptions: [], // 保存每道题选中选项的数组，初始化为null
      isCorrect: [], // 保存每道题是否答对的数组
      questionResults: {}, // 保存每道题是否答对的对象，键为题目id，值为bool
      questions: [{
        word_id: 1,
        word: 'abandon',
  ...
  methods: {
    ...
    selectOption(choiceIndex) {
      ...
      const questionId = this.questions[this.currentQuestionIndex].word_id;
      ...
      this.$set(this.questionResults, questionId, isCorrect); // 更新题目答对状态
    },
    ...
    nextQuestion() {
      if (this.selectedOptions[this.currentQuestionIndex] !== null && this.currentQuestionIndex < this.questions.length - 1) {
        ...
        uni.request({
          ...
          data: {
            punch_result: this.questionResults,
          },
          success: (res) => {
            console.log(res);
